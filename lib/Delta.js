...
const {
    ...
    getLength, hasAttributes, getContent,
...
function transformCursor(cursor, delta, isOwnDelta) {
    ...
    let offset = 0
    let operation = iterator.operation

    while (operation !== null && offset <= cursor) {
        ...
        } else if (isInsert(operation) && (offset < cursor || isOwnDelta)) {
            cursor += length
            offset += length
...
function toString(delta) {
    let result = ''

    for (let i = 0, l = delta.length; i < l; ++i) {
        const operation = delta[i]

        if (isInsert(operation)) {
            result += getContent(operation)
        } else {
...
function diffX(delta1, delta2) {
    ...
    const stringDiff = fastDiff(toString(delta1), toString(delta2))
    const iterator1 = new Iterator(delta1)
    const iterator2 = new Iterator(delta2)
    const callback = (operation1, operation2) => {
        append(result1, operation1)
        append(result2, operation2)
    }

    for (let i = 0, l = stringDiff.length; i < l; ++i) {
        const [ diffType, diffContent ] = stringDiff[i]

        diffIterators(iterator1, iterator2, diffType, diffContent, callback)
    }
