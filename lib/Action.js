...
var getAttributes = function getAttributes(action, attributeNames) {
  ...
  if (attributeNames) {
    ...
    while (i1 < l1 && i2 < l2) {
      var actionAttributeName = action[i1];
      var rawAttributeName = attributeNames[i2];
      var lastCharacterIndex = rawAttributeName.length - 1;
      var wildcard = lastCharacterIndex >= 0 && rawAttributeName[lastCharacterIndex] === '*';
      var attributeName = wildcard ? rawAttributeName.substring(0, lastCharacterIndex) : rawAttributeName;
      ...
      if (actionAttributeName < attributeName) {
        ...
        _attributes[i3++] = action[i1++];
      } else if (actionAttributeName === attributeName) {
