...
const slice = (operation, offset, count, length) => {
    if (offset === 0 && count === length) {
        ...
        return setContent(clone(operation, false), operation[CONTENT].substr(offset, count))
    } else {
        // it must be retain or delete
        return setContent(clone(operation, false), count)
    }
}
...
const merge = (operation1, operation2) => {
    ...
    switch (action) {
        ...
        case ACTION_RETAIN:
            ...
            return setContent(clone(operation1, false), operation1[CONTENT] + operation2[CONTENT])
...
const areTypesEqual = (operation1, operation2) =>
...
const composeIterators = (iterator1, iterator2) => {
    do {
        ...
        if (isRetain(operation2)) {
            ...
            const newOperation = clone(operation1, true)
