...
function composeSimilar(operation1, operation2) {
    ...
    if (newOperation.length !== operation1.length) {
        return null
    }

    for (let i = 0, l = newOperation.length; i < l; ++i) {
        if (!areTypesEqual(newOperation[i], operation1[i])) {
            return null
        }
    }
...
function applyAndInvert(snapshot, operation) {
    const editingPosition =
        !isNoop(operation) &&
        isRetain(operation[0]) &&
        !hasAttributes(operation[0]) ?
