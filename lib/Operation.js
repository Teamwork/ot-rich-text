const { copyArray, arraysEqual } = require('./util')
...
// ACTION_(INSERT_*|RETAIN) -> the index of the first attribute's name
const ATTRIBUTES = 2
...
const create = (action, content, attributes) => appendAttributes([ action, content ], attributes)
const createInsertText = (content, attributes) => create(ACTION_INSERT_TEXT, content || '', attributes)
const createInsertOpen = (content, attributes) => create(ACTION_INSERT_OPEN, content || '', attributes)
const createInsertClose = (content, attributes) => create(ACTION_INSERT_CLOSE, content || '', attributes)
const createInsertEmbed = (content, attributes) => create(ACTION_INSERT_EMBED, content || '', attributes)
const createRetain = (content, attributes) => create(ACTION_RETAIN, content > 0 ? content : 0 , attributes)
const createDelete = (content) => create(ACTION_DELETE, content > 0 ? content : 0)
...
const slice = (operation, offset = 0, count = MAX_INT) => {
    if (count <= 0) {
        // wrong count
        return isInsert(operation) ? setContent(copyArray(operation), '') : setContent(copyArray(operation), 0)
    }
    ...
    if (offset === 0 && count === length) {
        ...
        return setContent(copyArray(operation), getContent(operation).substr(offset, count))
    } else {
        // it must be retain or delete
        return setContent(copyArray(operation), count)
    }
}
...
const merge = (operation1, operation2) => {
    ...
    switch (action) {
        ...
        case ACTION_DELETE:
        case ACTION_RETAIN:
            for (let i = ATTRIBUTES; i < length; ++i) {
                if (operation1[i] !== operation2[i]) {
                    return null
                }
            }
            return setContent(copyArray(operation1), getContent(operation1) + getContent(operation2))
...
const composeIterators = (iterator1, iterator2) => {
    ...
    if (isRetain(operation2)) {
        ...
        if (operation2.length <= ATTRIBUTES || arraysEqual(operation1, operation2, ATTRIBUTES)) {
            // no attribute changes
            return slice(operation1, offset1, length)
        }

        const newOperation =
            isRetain(operation1) ? createRetain(length) :
            isInsertText(operation1) ? createInsertText(operation1[CONTENT].substr(offset1, length)) :
            create(operation1[ACTION], operation1[CONTENT])

        const keepNull = isRetain(operation1)
        let i1 = ATTRIBUTES // attribute index in operation1
        let i2 = ATTRIBUTES // attribute index in operation2
        let i3 = ATTRIBUTES // attribute index in newOperation
