...
const validate = operation => {
    ...
    switch (operation[ACTION]) {
        ...
        case ACTION_INSERT_CLOSE:
        case ACTION_INSERT_EMBED:
            return validateMinLength(operation, 5) ||
...
const getNodeName = operation => operation[NODE_NAME]
...
const slice = (operation, offset = 0, count = MAX_INT) => {
    if (count <= 0) {
        // wrong count
        return isInsert(operation) ? setContent(copyOperation(operation, false), '') : setContent(copyOperation(operation, false), 0)
    }
...
const composeIterators = (iterator1, iterator2) => {
    do {
        ...
        if (operation1 === null) {
            ...
            iterator2.next(getLength(operation2) - offset)
            return slice(operation2, offset)
        }

        if (operation2 === null) {
            const offset = iterator1.offset
            iterator1.next(getLength(operation1) - offset)
        ...
        const remaining1 = getLength(operation1) - offset1
        const remaining2 = getLength(operation2) - offset2
    ...
    switch (diffType) {
        case 1: // `diffContent` present only in iterator2
            ...
            while (diffLength > 0) {
                ...
                delta2Callback(slice(operation2, offset2, length))
            }
            break
        ...
        case -1: // `diffContent` present only in iterator1
            ...
            while (diffLength > 0) {
                ...
                delta1Callback(slice(operation1, offset1, length))
            }
            break
