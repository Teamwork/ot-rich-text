...
const createInsert = action => (content, author, attributes) => {
    let operationIndex = INSERT_ATTRIBUTES
...
const setAttribute = (operation, name, value) => {
    let insertAt = getAttributesIndex(operation)
    ...
    while (insertAt < operationLength) {
        previousName = operation[insertAt]
        previousValue = operation[insertAt + 1]
        operation[insertAt++] = nextName
        operation[insertAt++] = nextValue
...
const validate = operation => {
    ...
    switch (operation[ACTION]) {
        ...
        case ACTION_RETAIN:
            return validateMinLength(operation, RETAIN_ATTRIBUTES) ||
                validateContentNumber(operation) ||
                validateAttributes(operation, RETAIN_ATTRIBUTES, true)

        case ACTION_INSERT_TEXT:
            return validateMinLength(operation, INSERT_ATTRIBUTES) ||
                validateContentText(operation) ||
                validateAuthor(operation) ||
                validateAttributes(operation, INSERT_ATTRIBUTES, false)
        case ACTION_INSERT_OPEN:
        case ACTION_INSERT_CLOSE:
        case ACTION_INSERT_EMBED:
            return validateMinLength(operation, INSERT_ATTRIBUTES) ||
...
const getAuthor = operation => operation[AUTHOR] // for insert
...
const getAttributes = (operation, attributeNames) => { // for insert and retain
    if (attributeNames) {
        let i1 = getAttributesIndex(operation) // index for operation
