...
const createInsertText = (content, version, author, attributes) => {
...
const createInsertNode = action => (content, version, author, nodeName, attributes) => {
    const attributesLength = attributes ? attributes.length : 0
...
const validateLength = (operation, minLength) => {
...
const validateContentNodeId = operation => {
    const content = operation[CONTENT]

    if (typeof content !== 'string' || content.length !== 1) {
        return new Error('content must be a one character string')
    }

    const charCode = content.charCodeAt(0)
    const charCodeValid = 0xE000 <= charCode && charCode <= 0xF8FF

    if (!charCodeValid) {
        return new Error('content must be a character in the Private Use Area of the Basic Multilingual Unicode Plane')
    }
...
const validateNodeName = operation => {
    const nodeName = operation[NODE_NAME]

    if (typeof nodeName !== 'string') {
        return new Error('node name must be a string')
    }
...
const validate = operation => {
    ...
    switch (operation[ACTION]) {
        ...
        case ACTION_INSERT_TEXT:
            return validateLength(operation, 4) ||
        ...
        case ACTION_INSERT_CLOSE:
        case ACTION_INSERT_EMBED:
            return validateLength(operation, 5) ||
                validateContentNodeId(operation) ||
                validateVersion(operation) ||
                validateAuthor(operation) ||
                validateNodeName(operation) ||
                validateAttributes(operation, 5, false)
...
const getAttributesIndex = operation => {
    switch (operation[ACTION]) {
        case ACTION_INSERT_TEXT:
            return INSERT_TEXT_ATTRIBUTES
        case ACTION_INSERT_OPEN:
        case ACTION_INSERT_CLOSE:
        case ACTION_INSERT_EMBED:
            return INSERT_NODE_ATTRIBUTES
        case ACTION_RETAIN:
...
const getLength = operation => {
    switch (operation[ACTION]) {
        ...
        case ACTION_RETAIN:
        case ACTION_DELETE:
            const length = getContent(operation) | 0
...
const transformIterators = (iterator1, iterator2, operation1HasPriority) => {
    ...
    if (isInsert(operation2)) {
        const remaining = iterator2.remaining
    ...
    if (isDelete(operation2)) {
        ...
        return transformIterators(iterator1, iterator2, operation1HasPriority)
    }
...
const diffRetainBoth = (count, callback) => {
    const operation = createRetain(count)

    callback(operation, operation)
}

const diffDeleteBoth = (count, callback) => {
...
// `diffType` and `diffContent` params. Emits the results through `callback`.
//
...
// `callback`: called with 2 params:
//   param1: an operation which would convert the content of iterator2 to the content of iterator1, when applied
//   param2: an operation which would convert the content of iterator1 to the content of iterator2, when applied
const diffIterators = (iterator1, iterator2, diffType, diffContent, callback) => {
    ...
    switch (diffType) {
        case 1: // `diffContent` present only in iterator2
            while (diffLength > 0) {
                const operation2 = iterator2.operation
                const offset2 = iterator2.offset
                const remaining2 = iterator2.remaining
                const length = diffLength <= remaining2 ? diffLength : remaining2

                diffLength -= length
                iterator2.next(length)

                callback(createDelete(length), slice(operation2, offset2, length))
            }
            break
        ...
        case -1: // `diffContent` present only in iterator1
            while (diffLength > 0) {
                const operation1 = iterator1.operation
                const offset1 = iterator1.offset
                const remaining1 = iterator1.remaining
                const length = diffLength <= remaining1 ? diffLength : remaining1

                diffLength -= length
                iterator1.next(length)

                callback(slice(operation1, offset1, length), createDelete(length))
            }
            break
        ...
        case 0: // `diffContent` present in both iterator1 and iterator2
            while (diffLength > 0) {
                ...
                if (operation1[ACTION] === operation2[ACTION] &&
                    ...
                    (operation1[ACTION] === ACTION_INSERT_TEXT || operation1[NODE_NAME] === operation2[NODE_NAME])
                ) {
                    if (areAttributesEqual(operation1, operation2)) {
                        // This is going to be the most common case in practice.
                        // TODO optimize it!!!
                        diffRetainBoth(length, callback)
                        ...
                        while (i1 < l1 || i2 < l2) {
                            if (i1 >= l1) {
                                ...
                                // no more attributes is operation2
                        ...
                        callback(retain1, retain2)
                    }

                } else {
                    callback(slice(operation1, offset1, length), slice(operation2, offset2, length))
                    diffDeleteBoth(length, callback)
                }
            }
            break
    }
