...
const slice = (operation, offset, count) => {
...
const composeIterators = (iterator1, iterator2) => {
    do {
        ...
        if (operation1 === null) {
            ...
            const remaining = getLength(operation2) - offset
            iterator2.next(remaining)
            return slice(operation2, offset, remaining)
        }

        if (operation2 === null) {
            const offset = iterator1.offset
            const remaining = getLength(operation1) - offset
            iterator1.next(remaining)
            return slice(operation1, offset, remaining)
        }
    ...
    switch (diffType) {
        case 0: // `diffContent` present in both iterator1 and iterator2
            ...
            while (diffLength > 0) {
                ...
                const offset1 = iterator1.offset
                const remaining1 = operation1[CONTENT].length - offset1
                const operation2 = iterator2.operations[iterator2.index]
                const offset2 = iterator2.offset
                const remaining2 = operation2[CONTENT].length - offset2
                const length =
