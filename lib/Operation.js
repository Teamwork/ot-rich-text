...
function applyAndInvert(snapshot, operation) {
  ...
  var diffs = diffX(snapshot, newSnapshot, {
    oldRange: {
      index: editingPosition,
      length: 0
    },
    newRange: {
      index: editingPosition,
...
function diffX(snapshot1, snapshot2, editLocation) {
  ...
  } else if (editLocation && (oldRange = editLocation.oldRange || null)) {
    oldRange = {
      index: oldRange.index | 0,
      length: oldRange.length | 0
    };

    if (newRange = editLocation.newRange || null) {
      newRange = {
        index: newRange.index | 0,
        length: newRange.length | 0
      };
    }
