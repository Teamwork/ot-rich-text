...
const create = (action, content, attributes) => {
    ...
    if (ACTION_INSERT_TEXT <= action && action <= ACTION_INSERT_EMBED) {
        operation[operationIndex++] = 1
        operation[operationIndex++] = 'user'

        if (ACTION_INSERT_OPEN <= action && action <= ACTION_INSERT_EMBED) {
            operation[operationIndex++] = 'DIV'
        }
    }
